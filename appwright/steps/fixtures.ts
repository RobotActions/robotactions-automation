 ...
 *
 * It is `playwright-bdd`'s test (itself Playwright's, extended) plus one
 * fixture: `device`, an Appwright `Device` bound to a session on the grid. BDD
 * steps and plain specs therefore share one device lifecycle, and the Appwright
 * API is the real thing rather than a wrapper:
 *
...
export const test = base.extend<GridDeviceOptions & { device: Device }>({
    // Options — overridable per project in appwright.config.ts, and per file
    ...
    device: async (
        ...
        const provider = new RobotActionsDeviceProvider({
            platform,
            udid,
            buildPath: build,
        ...
        testInfo.annotations.push({ type: 'sessionId', description: provider.sessionId });
        await provider.syncTestDetails({ name: `${testInfo.title} [${platform}]` });
