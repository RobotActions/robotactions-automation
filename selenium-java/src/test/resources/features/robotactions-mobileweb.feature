...
Feature: RobotActions on mobile web (Appium, real device browser)
  ...
  # Step wording is deliberately browser-neutral: Hooks stamps the scenario name
  # onto the grid session, so Android-specific wording would mislabel every iOS
  # run in grid history.
  ...
  # Run with:  PLATFORM=mobileweb mvn test -Dtest=RunCucumberTest \
  #              -Dcucumber.filter.tags="@mobileweb and @smoke"
  #
  # Mobile-only behaviour these scenarios pin down (verified against the live
  # site at a 393px viewport):
  ...
  # ── Responsive layout ────────────────────────────────────────────────
  # The single most valuable mobile-only assertion: nothing may overflow the
  # viewport horizontally, or the user can pan the page sideways.
  @smoke
  ...
  Scenario Outline: Sub-pages fit the device viewport
    ...
    Then the mobile page title should contain "<titleFragment>"
  ...
  # ── Mobile menu ──────────────────────────────────────────────────────
  # At mobile width `nav a` yields 0 visible links; the hamburger reveals them.
  @smoke
  ...
  Scenario Outline: Tapping a menu link scrolls to its section and closes the menu
    ...
    And the mobile section with id "<id>" should be in viewport
  ...
  # deterministic across devices — verified 2026-08-04, the first question was
  # already expanded on a real Android handset and collapsed on a real iPhone.
  @sanity
