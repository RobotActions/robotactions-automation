...
Feature: RobotActions on mobile web (Appium, real device browser)
  ...
  browser is chosen by the config, not by the scenario:
  #   npx wdio run wdio.mobileweb.conf.ts      → Chrome on a real Android device
  #   npx wdio run wdio.ios-mobileweb.conf.ts  → Safari on a real iOS device
  ...
  # Run with:
  #   npx wdio run wdio.mobileweb.conf.ts --cucumberOpts.tagExpression='@smoke'
  #
