...
Feature: RobotActions homepage — full nav coverage + load profile
  ...
  # Run with:   --grep @smoke  (or @sanity, @regression)
  ...
  Scenario Outline: Top-level nav anchors scroll to the right section
    ...
    # Only these four are anchor links. "Products" and "Resources" are dropdown
    # BUTTONS, not links — clicking them by role=link never resolves. They are
    # covered by the dropdown scenarios below.
    Examples:
      | link    | fragment  | id       |
      | Home    | #hero     | hero     |
  ...
  Scenario: Pricing section shows expected tier headings
    ...
    # Tiers verified against the live site 2026-08-05: Free / Manual /
    # Automation / Enterprise. The previous "Shared" heading no longer exists
    # ("Shared" still appears in body copy, but not as a pricing heading).
